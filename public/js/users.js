...
// 修复: 渲染锁 + users 缓存 — 消除 Tab 切换时的竞态条件

let teamTab = 'members';
let _cachedUsers = null;  // 缓存 users 数据，Tab 切换不重复 fetch
let _renderSeq = 0;       // 渲染序号，防止并发覆盖

function switchTeamTab(tab) {
  if (tab === teamTab) return; // 重复点击忽略
  teamTab = tab;
  // @alpha: 用缓存数据同步渲染，不走网络 — 消除竞态根因
