...
function loadOpsLog(nodeId: any) {
  try {
    return JSON.parse(fs.readFileSync(path.join(OPS_LOG_DIR, `${nodeId}.json`), 'utf-8'));
  } catch (_) { return []; }
}

function saveOpsLog(nodeId: any, role: any, content: any) {
...
function loadAllOpsLogs() {
  try {
    const files = fs.readdirSync(OPS_LOG_DIR).filter((f: any) => f.endsWith('.json'));
...
async function boot() {
  ...
  app.use('/api/jobs', createJobsRouter({
    ...
    broadcastWS: (msg: any) => {
      ...
      wss.clients.forEach((ws: any) => { if (ws.readyState === 1) ws.send(data); });
    },
  }));
  ...
  function _resolveWsToken(token: any) {
