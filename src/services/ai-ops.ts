...
class AiOps {
  ...
  async _handleExec(msg: string) {
    ...
    if (spaceIdx < 0) return { response: '格式: exec <节点ID> <命令>', commands: [] as string[] };
  ...
  async _handleAsyncExec(msg: string) {
    ...
    const spaceIdx = parts.indexOf(' ');
    if (spaceIdx < 0) return { response: '格式: async exec <节点ID> <命令>', commands: [] as string[] };
